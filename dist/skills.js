...
async function* walkSkills(root, depth = 0) {
    if (depth > 4)
        return;
    let entries;
    try {
...
export async function discoverSkills(extraRoots = []) {
    ...
    for (const root of roots) {
        ...
        for await (const file of walkSkills(root)) {
            ...
            try {
                ...
                const name = String(data.name || path.basename(dir) || "skill").trim();
                const description = String(data.description || "").trim();
                if (!description)
                    continue; // require description to be useful
                let allowed;
...
export function buildSkillsPrompt(skills) {
    ...
    const lines = [];
    lines.push("## Skills");
    lines.push("A skill is a packaged set of local instructions stored in a SKILL.md file. " +
    ...
    lines.push("- Trigger: If the user names a skill (e.g. `$SkillName` or by plain text) " +
        "OR the task clearly matches a skill's description, you should use that skill for the turn.");
    lines.push("- Loading: Use the `read` tool to open the skill's SKILL.md file shown above before following its instructions.");
    lines.push("- Progressive disclosure: SKILL.md may reference relative paths like `scripts/`, `references/`, or `assets/`. " +
        "Resolve them relative to the skill directory, and load only the files you need.");
    lines.push("- If a skill defines `allowed-tools`, treat that as the preferred toolset for that skill.");
    lines.push("- If multiple skills apply, briefly state which ones you'll use and in what order.");
    lines.push("- If a named skill is missing or unreadable, say so briefly and continue with the best fallback.");
    return lines.join("\n");
}
