...
function defaultOnEvent(e) {
    switch (e.type) {
        ...
        case "assistant_text":
            ...
            break;
        case "tool_call":
            stdout.write(`${C.magenta}⏵ ${e.name}${C.reset} ${C.dim}${fmtInput(e.input)}${C.reset}\n`);
            break;
        case "tool_result": {
        ...
        case "turnEnd":
            stdout.write(`${C.dim}turn ${e.n} done · ${e.tools} tools · ${(e.durationMs / 1000).toFixed(1)}s · ↑${e.tokIn} ↓${e.tokOut}${C.reset}\n`);
            break;
        case "usage":
            break;
        case "stop":
            break;
        case "compact":
            stdout.write(`${C.yellow}🗜 compact:${C.reset} saved ~${Math.round(e.savedTokens / 1000)}K tokens
`);
            break;
    }
}
...
async function runSlash(line, state) {
...
async function runCommand(cmd, args, state) {
    const ctx = {
        args,
        raw: args.join(" "),
        cwd: process.cwd(),
...
function showMenu() {
    ...
    for (const c of SLASH_COMMANDS) {
        stdout.write(`  ${C.cyan}/${c.name.padEnd(w)}${C.reset}  ${C.dim}${c.description}${C.reset}\n`);
    }
    stdout.write(`${C.dim}Tip: press ${C.reset}${C.cyan}Tab${C.reset}${C.dim} after typing a prefix to autocomplete.${C.reset}\n`);
}
function maybeSuggest(rl) {
    const buf = rl.line ?? "";
