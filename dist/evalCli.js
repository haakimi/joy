...
export async function runEvalCli(opts = {}) {
    ...
    for (const testCase of selected) {
        ...
        if (opts.json) {
            ...
            if (result.status === "passed") {
                ...
                if (result.verify.stdout.trim())
                    write(`  stdout: ${result.verify.stdout.trim()}\n`);
                if (result.verify.stderr.trim())
                    write(`  stderr: ${result.verify.stderr.trim()}\n`);
                write(`  work dir: ${result.workDir}\n`);
            }
        }
    }
...
async function printEvalList(cases, write, json) {
    if (json) {
...
function normalizeProviderArg(value) {
    const v = value.toLowerCase();
    if (v === "anthropic" || v === "mock" || v === "glm")
        return v;
    return "mock";
}
