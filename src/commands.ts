...
export interface CommandContext {
  args: string[];
  raw: string;                          // full text user typed, without leading "/"
  cwd: string;
  model: string;
...
export interface CommandResult {
  /** if set, REPL will pass this prompt to the agent as a user turn */
  prompt?: string;
  /** exit the REPL */
  exit?: boolean;
  /** clear the screen before printing the next prompt */
  clear?: boolean;
}
...
export const SLASH_COMMANDS: SlashCommand[] = [
  ...
  },
  {
    ...
    run: async () => ({
      prompt:
        "Please explore the current project directory. Look at the top-level files, " +
  ...
  },
  {
    ...
    run: async ({ args, model, printLine }) => {
      if (args.length === 0) {
        ...
        printLine(
          `${C.dim}common: claude-sonnet-4-6, claude-haiku-4-5, claude-opus-4-8${C.reset}`,
        );
        return {};
      }
  ...
  },
  {
    ...
    run: async ({ printLine }) => {
      printLine(
        `${C.yellow}Compressing conversation history...${C.reset}`,
      );
      // The actual compression is handled by the REPL/App via the prompt mechanism
      return {
        prompt:
          "[COMPACT] Please summarize the conversation so far. Include the user's original request, " +
