...
type IgnoreFilter = { ignores(relativePath: string): boolean };
...
export const tools: ToolDef[] = [
  ...
  },
  {
    name: "apply_patch",
    description:
      "Apply a unified diff patch to UTF-8 text files. " +
...
const ignoreCache = new Map<string, Promise<IgnoreFilter>>();
const ALWAYS_IGNORED = new Set([".git", "node_modules", "dist", "coverage", ".next", ".cache"]);

async function buildIgnoreFilter(root: string): Promise<IgnoreFilter> {
  const ig = ignore();
  // Built-in fallback: even without a .gitignore these are always skipped.
  for (const d of ALWAYS_IGNORED) ig.add(`/${d}/`);
  ...
  if (ancestorPatterns.length) ig.add(ancestorPatterns.join("\n"));
...
async function collectNestedGitignores(
  root: string,
  relDir = "",
): Promise<Array<{ relDir: string; text: string }>> {
  const out: Array<{ relDir: string; text: string }> = [];
...
function rebasePatterns(text: string, relDir: string): string {
  if (!relDir) return text;
  return text
    .split(/\r?\n/)
    .map((line) => {
      ...
      if (trimmed.startsWith("!")) return `!${relDir}/${trimmed.slice(1)}`;
      if (trimmed.startsWith("/")) return `/${relDir}${trimmed}`;
      return `${relDir}/${trimmed}`;
    })
    .join("\n");
}

async function getIgnoreFilter(root: string): Promise<IgnoreFilter> {
  ...
  async function visit(dir: string): Promise<void> {
    ...
    for (const child of children) {
      ...
      if (child.isDirectory()) {
        if (shouldSkipDir(rel, child.name)) continue;
...
function validateCreateHunk(hunk: PatchHunk, filePath: string): void {
  if (hunk.oldCount !== 0) {
    throw new Error(
      `Invalid patch: create hunk for ${filePath} must have old count 0 (got ${hunk.oldCount}); use a normal hunk to edit an existing file`,
    );
  }
  if (hunk.lines.some((line) => line.kind !== "add")) {
    throw new Error(
      `Invalid patch: create hunk for ${filePath} must contain only additions; use a normal hunk to edit an existing file`,
    );
  }
}
...
function assembleCreatedFile(patchFile: PatchFile): string {
  const allAddLines: string[] = [];
  for (const hunk of patchFile.hunks) {
    for (const line of hunk.lines) {
      // Only add lines exist in create hunks (validated by validateCreateHunk).
      if (line.kind === "add") allAddLines.push(line.text);
    }
  }
...
async function applyPatchTool(input: Record<string, unknown>): Promise<{ content: string; is_error: boolean }> {
  ...
  const updates: Array<{ abs: string; text: string; create: boolean }> = [];
  let added = 0;
  let removed = 0;
  let hunkCount = 0;
  let createdCount = 0;
  ...
  for (const file of files) {
    ...
    if (file.mode === "create") {
      ...
      if (exists) {
        throw new Error(
          `Cannot create file: ${file.path} already exists; use a normal hunk to edit an existing file`,
        );
      }
